...
class SOATest(unittest.TestCase):
    ...
    def check_output_mock(self, command):
        ...
        assert normalized_command in self._stubbed_commands, \
            "Missing output for <%s>, only have output for <%s>" % (
                normalized_command, self._stubbed_commands)
        import inspect
    ...
    def provide_output(self, stubbed_output):
        "Outdents output"
        output = '\n'.join(map(lambda each: each.lstrip(),
                               stubbed_output.split('\n')))
    ...
    def test_get_soa_for_domain_from_nameserver(self):
        ...
        expect(
            soa) == 'nsa1.schlundtech.de. sh.sntl-publishing.com. 2014090302 43200 7200 1209600 600'
    ...
    def test_should_show_critical_error_if_hidden_primary_is_dead(self):
        ...
        self.on_command(
            'dig +short SOA yeepa.de @zhref-mail.zms.hosting').provide_function(fail)
    ...
    def test_should_return_false_if_soas_differ(self):
        self.on_command('dig +short NS yeepa.de').provide_output("""\
            ...
            nsb1.schlundtech.de.""")
        self.on_command(
            'dig +short SOA yeepa.de @nsc1.schlundtech.de').provide_output("not equal")
        self.on_command(
            'dig +short SOA yeepa.de @nsb1.schlundtech.de').provide_output("to this")
        expect(check_soas_equal_for_domain('yeepa.de')) == (NAGIOS.CRITICAL,
                                                            'Nameservers do not agree for domain "yeepa.de" [\'not equal\', \'to this\']')
    ...
    def test_should_erorr_if_nameservers_are_not_authoritative(self):
        self.on_command('dig +short NS example.com').provide_output("""\
            ...
            a.iana-servers.net.""")
        self.on_command(
            'dig +short SOA example.com @a.iana-servers.net').provide_output("anything")
        self.on_command(
            'dig +short SOA example.com @b.iana-servers.net').provide_output("")
        expect(check_soas_equal_for_domain('example.com')) == (NAGIOS.CRITICAL,
                                                               'Nameserver(s) [\'b.iana-servers.net\'] did not return SOA record for domain "example.com"')
    ...
    def test_should_allow_to_configure_warning_level_for_number_of_nameservers(self):
        self.on_command('dig +short NS yeepa.de').provide_output("""\
            ...
            nsb1.schlundtech.de.""")
        self.on_command(
            'dig +short SOA yeepa.de @nsc1.schlundtech.de').provide_output("equal")
        self.on_command(
            'dig +short SOA yeepa.de @nsb1.schlundtech.de').provide_output("equal")
    ...
    def test_should_error_if_less_than_critical_nameservers(self):
        self.on_command(
            'dig +short NS yeepa.de').provide_output("""nsc1.schlundtech.de.""")
        self.on_command(
            'dig +short SOA yeepa.de @nsc1.schlundtech.de').provide_output("good enough")
    ...
    def test_should_count_non_answering_nameserver_as_empty_response(self):
        ...
        def fail(): raise subprocess.CalledProcessError(-1, 'dig ...', "process error'd")
        self.on_command(
            'dig +short SOA yeepa.de @nsb1.schlundtech.de').provide_function(fail)
        expect(check_soas_equal_for_domain('yeepa.de', )) == (
